<?php
class Inventory {
	private $host = 'ims.c3wwm9013gwd.ap-south-1.rds.amazonaws.com';
	private $user = 'root';
	...
	public function getUserByEmail($email) {
        $email = $this->dbConnect->real_escape_string($email);
        ...
        if ($result) {
            if ($result->num_rows > 0) {
                // Assuming your users table has columns like 'userid', 'name', etc.
	...
	public function registerUser($name, $email, $password) {
        $name = $this->dbConnect->real_escape_string($name);
        $email = $this->dbConnect->real_escape_string($email);

        // You should hash the password for security. Use password_hash function.
        $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

        $query = "INSERT INTO " . $this->userTable . " (name, email, password) VALUES ('$name', '$email', '$hashedPassword')";
